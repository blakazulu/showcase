...
- **`ProjectList`** — the terminal output region (`data-testid="project-list"`); renders one `ProjectRow` per project in the incoming (shuffled) order and pre-expands the **first** row (`open={i === 0}`), so the open panel is always at the top and which project it is stays random per visit.
...
- List order: `HomeFilter` shuffles per visit, filtered by the active chip; `ProjectList` pre-expands whichever project is first in that order.
